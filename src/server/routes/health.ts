...
export async function registerHealthRoutes(app: FastifyInstance, cfg: AppConfig) {
  app.get('/api/health', async () => {
    return {
      ...
      telegramWebhookConfigured: Boolean(cfg.telegram.webhookSecret),
      telegramBotConfigured: Boolean(cfg.telegram.botToken),
  ...
  app.get('/api/vapidPublicKey', async (_req, reply) => {
    if (!cfg.vapid.enabled || !cfg.vapid.publicKey) {
      return reply.code(503).send({
        ok: false,
        error:
          'Push is not configured on the server. Set VAPID_SUBJECT, VAPID_PUBLIC_KEY, and VAPID_PRIVATE_KEY.',
      });
    }
