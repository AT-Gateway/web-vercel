...
export async function registerAndroidRoutes(
    ...
    app.post("/api/android/inbound-sms", async (req, reply) => {
        ...
        const plaintext =
            body.body !== undefined && body.body !== null ? String(body.body) : "";
        const enc = body.enc;

        const ts = typeof body.ts === "number" ? Number(body.ts) : Date.now();
        const simSlotIndex =
            body.simSlotIndex === 0 || body.simSlotIndex === 1
                ? Number(body.simSlotIndex)
                : null;
        const subscriptionId =
            typeof body.subscriptionId === "number" ? Number(body.subscriptionId) : null;

        if (!gatewayDeviceId || !from) {
            return reply
                .code(400)
                .send({ ok: false, error: "Missing gatewayDeviceId or from" });
        }
        ...
        if (insert.inserted) {
                    ...
                    for (const s of subs) {
                        try {
                            ...
                            if (err?.statusCode === 404 || err?.statusCode === 410) {
                                await repo
                                    .deletePushSubscription(pairingId, s.deviceId)
                                    .catch(() => {});
                            }
                        }
                    }
            ...
            if (telegramCfgs.length > 0) {
                try {
                    ...
                    const simLabel =
                        simSlotIndex === 0
                            ...
                            : simSlotIndex === 1
                              ? "SIM2"
                              : "AUTO";
                    const header = `📩 [${gatewayDeviceId}] ${simLabel} — ${title}`;
                    const msg = `${header}\n\n${text}`;

                    // Inline buttons: reply context (uses message id).
                    const replyMarkup = {
                        inline_keyboard: [
                            [
                    ...
                    await Promise.all(
                        telegramCfgs.map((c) => telegramSend(c, msg, { replyMarkup }))
                    );
                } catch {
                    // ignore
                }
    ...
    app.get("/api/android/outbox", async (req, reply) => {
        ...
        if (!gatewayDeviceId)
            return reply.code(400).send({ ok: false, error: "Missing gatewayDeviceId" });
    ...
    app.post("/api/android/outbox/status", async (req, reply) => {
        ...
        if (!id || (status !== "sent" && status !== "failed")) {
            return reply
                .code(400)
                .send({ ok: false, error: "Missing id or invalid status" });
        }
        ...
        if (meta.pairingId) {
            ...
            hub.emit(meta.pairingId, "status", {
                id,
                status,
                peer: meta.peer,
                threadId: tail || norm || meta.peer,
            });
        }
    ...
    app.post("/api/android/outbox/delivered", async (req, reply) => {
        ...
        const id = String(body.id ?? "").trim();
        const deliveredAt =
            typeof body.deliveredAt === "number" ? Number(body.deliveredAt) : Date.now();
